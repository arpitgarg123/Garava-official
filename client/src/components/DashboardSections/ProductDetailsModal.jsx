        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              ...
              {/* Hero Image */}
              <div className="mb-4">
                <p className="text-[1.0625rem] text-gray-600 mb-2">Hero Image</p>
              ...
              {product.gallery && product.gallery.length > 0 && (
                <div>
                  <p className="text-[1.0625rem] text-gray-600 mb-2">Gallery ({product.gallery.length} images)</p>
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-[1.0625rem] text-gray-600">Product Name</p>
                  <p className="font-medium text-lg">{product.name}</p>
                </div>

                <div>
                  <p className="text-[1.0625rem] text-gray-600">Slug</p>
                  <p className="text-gray-900">{product.slug}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[1.0625rem] text-gray-600">Type</p>
                  ...
                  <div>
                    <p className="text-[1.0625rem] text-gray-600">Category</p>
                ...
                {product.subcategory && (
                  <div>
                    <p className="text-[1.0625rem] text-gray-600">Subcategory</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[1.0625rem] text-gray-600">Status</p>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[1.0625rem] font-medium border ${getStatusColor(product.status)}`}>
                  ...
                  <div>
                    <p className="text-[1.0625rem] text-gray-600">Total Stock</p>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[1.0625rem] font-medium border ${getStatusColor(getStockStatus(getTotalStock()))}`}>
                ...
                <div>
                  <p className="text-[1.0625rem] text-gray-600">Rating</p>
                  <div className="flex items-center">
                ...
                {product.tags && product.tags.length > 0 && (
                  <div>
                    <p className="text-[1.0625rem] text-gray-600 mb-2">Tags</p>
                    <div className="flex flex-wrap gap-2">
                      {product.tags.map((tag, index) => (
                        <span key={index} className="px-2 py-1 bg-gray-50 text-gray-700 text-[1.0625rem] rounded">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex gap-4">
                  {product.isFeatured && (
                    <span className="px-2 py-1 bg-blue-100 text-blue-800 text-[1.0625rem] rounded font-medium">
                      Featured
                    </span>
                  )}
                  {product.isActive && (
                    <span className="px-2 py-1 bg-green-100 text-green-800 text-[1.0625rem] rounded font-medium">
                      Active
                    </span>
                  )}
                </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {product.variants.map((variant, index) => (
                      <tr key={index} className={variant.isDefault ? 'bg-blue-50' : 'bg-white'}>
                        ...
                        <td className="px-4 py-3 text-[1.0625rem] text-gray-900">{formatCurrency(variant.price)}</td>
                        <td className="px-4 py-3 text-[1.0625rem] text-gray-900">
                          {variant.mrp ? formatCurrency(variant.mrp) : '-'}
                        </td>
                        <td className="px-4 py-3 text-[1.0625rem]">
                          <span className={`px-2 py-1 rounded-full text-[1.0625rem] font-medium ${getStatusColor(getStockStatus(variant.stock))}`}>
                            {variant.stock || 0}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-[1.0625rem] text-gray-900">
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {product.shortDescription && (
                <div>
                  <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Short Description</p>
              ...
              {product.description && (
                <div>
                  <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Full Description</p>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {product.fragranceNotes.top && product.fragranceNotes.top.length > 0 && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Top Notes</p>
                    <div className="flex flex-wrap gap-1">
                      {product.fragranceNotes.top.map((note, index) => (
                        <span key={index} className="px-2 py-1 bg-yellow-100 text-yellow-800 text-[1.0625rem] rounded">
                          {note}
                        </span>
                      ))}
                    </div>
                ...
                {product.fragranceNotes.middle && product.fragranceNotes.middle.length > 0 && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Middle Notes</p>
                    <div className="flex flex-wrap gap-1">
                      {product.fragranceNotes.middle.map((note, index) => (
                        <span key={index} className="px-2 py-1 bg-green-100 text-green-800 text-[1.0625rem] rounded">
                          {note}
                        </span>
                      ))}
                    </div>
                ...
                {product.fragranceNotes.base && product.fragranceNotes.base.length > 0 && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Base Notes</p>
                    <div className="flex flex-wrap gap-1">
                      {product.fragranceNotes.base.map((note, index) => (
                        <span key={index} className="px-2 py-1 bg-purple-100 text-purple-800 text-[1.0625rem] rounded">
                          {note}
                        </span>
                      ))}
                    </div>
              ...
              {(product.ingredients || product.caution || product.storage) && (
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                  {product.ingredients && (
                    <div>
                      <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Ingredients</p>
                      <p className="text-gray-900 bg-gray-50 p-3 rounded text-[1.0625rem]">{product.ingredients}</p>
                    </div>
                  )}

                  {product.caution && (
                    <div>
                      <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Caution</p>
                      <p className="text-gray-900 bg-gray-50 p-3 rounded text-[1.0625rem]">{product.caution}</p>
                    </div>
                  )}

                  {product.storage && (
                    <div>
                      <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Storage</p>
                      <p className="text-gray-900 bg-gray-50 p-3 rounded text-[1.0625rem]">{product.storage}</p>
                    </div>
                  )}
                </div>
              )}
          ...
          {product.type === 'jewellery' && (product.dimensions || product.material || product.careInstructions) && (
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {product.dimensions && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Dimensions</p>
                ...
                {product.material && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Material</p>
                ...
                {product.careInstructions && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Care Instructions</p>
            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {product.collections && product.collections.length > 0 && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Collections</p>
                    <div className="flex flex-wrap gap-2">
                      {product.collections.map((collection, index) => (
                        <span key={index} className="px-3 py-1 bg-indigo-100 text-indigo-800 text-[1.0625rem] rounded-full">
                          {collection}
                        </span>
                      ))}
                    </div>
                ...
                {product.badges && product.badges.length > 0 && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Badges</p>
                    <div className="flex flex-wrap gap-2">
                      {product.badges.map((badge, index) => (
                        <span key={index} className="px-3 py-1 bg-orange-100 text-orange-800 text-[1.0625rem] rounded-full">
                          {badge}
                        </span>
                      ))}
                    </div>
          ...
          {(product.metaTitle || product.metaDescription) && (
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {product.metaTitle && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Meta Title</p>
                ...
                {product.metaDescription && (
                  <div>
                    <p className="text-[1.0625rem] font-medium text-gray-700 mb-2">Meta Description</p>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[1.0625rem] text-gray-600">
              <div>
