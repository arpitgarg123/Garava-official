        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-4">
              <span className={`px-3 py-1 rounded-full text-md font-medium ${getStatusColor(blog.status)}`}>
                {blog.status}
              </span>
              {blog.category && (
                <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-md font-medium">
                  {blog.category}
                </span>
              )}
            </div>
              ...
              {blog.tags && blog.tags.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {blog.tags.map((tag, index) => (
                    <span
                      key={index}
                      className="px-2 py-1 bg-gray-50 text-gray-700 rounded text-md"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <p className="text-gray-500 text-md">No tags assigned</p>
              )}
            </div>
          </div>
