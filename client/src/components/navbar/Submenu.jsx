...
const Submenu = ({ sub, parentTitle, isMobile = false, onClickItem = () => {} }) => {
  ...
  const Content = () => (
    <>
      {isTextOnly ? (
        <div className="bg-gray-200 text-black flex items-center justify-center h-28 w-28 sm:h-32 sm:w-32 mb-2 text-center font-semibold text-sm">
          {sub.label}
        </div>
      ) : (
        <img
      ...
      {!isTextOnly && (
        <span className="text-sm font-medium group-hover:text-primary text-center">{sub.label}</span>
      )}
    </>
  );
