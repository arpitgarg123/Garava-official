...
const OrderStatus = ({ status }) => {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-md ${getColor()}`}>
      {getDisplayText()}
    </span>
  );
};
                ...
                {orderDetails.items?.map((item, index) => (
                  <div key={index} className="flex space-x-4 border-b pb-4 last:border-b-0">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-gray-500 text-md">Quantity: {item.quantity}</p>
                    </div>
                    <div className="text-right">
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-md text-gray-500">Order #{order.orderNumber}</p>
                  ...
                  {order.items?.slice(0, 2).map((item, idx) => (
                    <div key={idx} className="flex space-x-4 hover:bg-gray-50 p-2 rounded">
                      ...
                      <div>
                        ...
                        <p className="text-md text-gray-500 my-2">Qty: {item.quantity}</p>
                        <p className="text-md">{formatCurrency(item.unitPrice)}</p>
                      </div>
                    </div>
                  ))}
                  {order.items?.length > 2 && (
                    <p className="text-md text-gray-500 pl-24">
